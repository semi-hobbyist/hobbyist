...
@WebServlet("/myCartDelete")
public class MyCartDeleteAjaxServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public MyCartDeleteAjaxServlet() {
        super();
    }

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String delete_temp = request.getParameter("deleteNo");
		String deleteNo[] = delete_temp.split(",");
		// 해당 장바구니 번호
		System.out.println(delete_temp);
		String member = request.getParameter("member");
		// 해당 멤버아이디(이메일)
		System.out.println("들어온 이메일 : " + member);
		
		// 카트 카테고리 받아오기
		String cartCate = request.getParameter("cartCate");
		System.out.println("들어온 카테고리 : " + cartCate);
		
		// 해당번호 장바구니 지우기
		// 선택삭제이기 때문에 값이 여러개여서 반복문으로 지운다
		int result = 0;
		for(String s : deleteNo) {
			result += new MyCartService().deleteCart(Integer.parseInt(s));
			System.out.println("[DELETE 장바구니]" + result + "번 장바구니에서 삭제");
		}
		
		// 삭제 완료 후, 장바구니 리스트를 다시 불러온다
		...
		// 삭제 완료 후, myCart_Ajax 테이블을 갱신해준다
		String html = "";
		
		// 분기처리 ... ㅠㅠㅠ
		if(cartCate.equals("classshop")) {
			if(!list.isEmpty()) {
				html += "<tr>";
				...
				for(MyCart mc : list) {
					for(Shop s : shop) {
						if(mc.getMyCartClass()==s.getShopNo()) {
							...
							html += "<td>";
							html += "<input type=\"hidden\" id=\"classOption\"name=\"classOption\" value=\"" + mc.getMyCartClassOption() + "\">" + mc.getMyCartClassOption() + "<br></td>";
