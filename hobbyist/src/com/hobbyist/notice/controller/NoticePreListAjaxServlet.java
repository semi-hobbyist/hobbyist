...
/**
 * Servlet implementation class NoticePreListAjaxServlet
 */
@WebServlet("/notice/noticePreListAjax.do")
public class NoticePreListAjaxServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public NoticePreListAjaxServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(list.size() != 0) {
			for(int i=0;i<list.size();i++) {
				...
				html += "</div>";
				html += "<div class='talC_noticeTitle'onclick='fn_defaultViewAjax("+ list.get(i).getNoticeNo() +")'>" + list.get(i).getNoticeTitle() + "</div>";
				...
				//남은기간 계산
				int noticeNo = list.get(i).getNoticeNo();
				int remainTime = new NoticeService().remainTime(noticeNo); 
				html += remainTime + "일 남음";
